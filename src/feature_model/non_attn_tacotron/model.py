...
from synthesizer.utils import get_mask_from_lengths, create_pos_enc
...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths, speaker_embedding):
        for conv in self.convolutions:
            x = F.dropout(conv(x), 0.5, self.training)  # [B, 512, L]
...
class DurationPredictor(nn.Module):
    def __init__(self, hparams):
        ...
        self.lstm = nn.LSTM(hparams.encoder_embedding_dim * 2 + hparams.speaker_embedding_size,
                            hparams.encoder_embedding_dim, 2,
                            batch_first=True, bidirectional=True)
...
class RangePredictor(nn.Module):
    def __init__(self, hparams):
        ...
        self.lstm = nn.LSTM(hparams.encoder_embedding_dim * 2 + hparams.speaker_embedding_size + 1,
                            hparams.encoder_embedding_dim, 2,
                            batch_first=True, bidirectional=True)
        self.projection = nn.Linear(
            hparams.encoder_embedding_dim*2, 1)
    ...
    def forward(self, x, input_lengths):
        ...
        #     input_lengths.unsqueeze(-1)).transpose(1, 2)
        input_lengths = input_lengths.cpu().numpy()
        x = nn.utils.rnn.pack_padded_sequence(
            x, input_lengths, batch_first=True)
...
class GaussianUpsample(nn.Module):
    ...
    def forward(self, encoder_outputs, durations, range_outputs, device='cuda'):
        ...
        w_2 = torch.sum(torch.exp(-((range_outputs)**-2) *
                                  ((t-c) ** 2)), dim=1, keepdim=True) + 1e-20
        w = w_1/w_2  # [B, L, T]
        # w[w != w] = 0  # n?an_to_num
        ...
        pos_enc_inp = torch.zeros(durations.size(0), torch.max(x).item())
        pos_enc_inp = create_pos_enc(
            pos_enc_inp, durations.cpu().numpy()).to(device)

        return pos_enc_inp.unsqueeze(-1), out
...
class NonAttentiveTacotron(nn.Module):
    ...
    def parse_batch(self, batch):
        ...
        return ((text, src_len, mel_target, mel_len, durations, max_src_len, max_mel_len, embeds), (mel_target, durations), idx)

    def parse_output(self, outputs, output_lengths=None, src_len=None):
        if self.mask_padding and output_lengths is not None:
            ...
            outputs[2].data.masked_fill_(src_mask, 0.0)
    ...
    def forward(self, inputs):
        ...
        return self.parse_output([mel_outputs, mel_outputs_postnet, duration_outputs.squeeze(-1)], mel_len, src_len)

    def inference(self, inputs, embeds, durations=None):
        ...
        duration_outputs = self.duration_predictor.inference(
            encoder_outputs)  # [B, L, 1]
        range_inputs = torch.cat(
            (encoder_outputs, duration_outputs), dim=2)  # [B, L, 1281]
        range_outputs = self.range_predictor.inference(
            range_inputs)  # [B, L, 1]
        ...
        mel_outputs_postnet = self.postnet(mel_outputs)
        mel_outputs_postnet = mel_outputs + mel_outputs_postnet
        # print(mel_outputs_postnet.size(), mel_outputs.size())

        return self.parse_output([mel_outputs, mel_outputs_postnet, duration_outputs])
